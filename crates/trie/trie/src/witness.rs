...
pub fn next_root_from_proofs(
    ...
    let mut trie_nodes = trie_nodes.into_iter().collect::<Vec<(_, _)>>();
    ...
    while let Some(key_nibbles) = keys.next() {
        if let Some((next_nibbles, _)) = keys.peek() {
            if next_nibbles.starts_with(&key_nibbles.0) {
                ignored.insert(key_nibbles.0.clone());
            }
        }
    }
