...
// /// Macro that generates snapshot creation functions that take an arbitratry number of [`Table`]
...
//         $(
//             paste::item! {
//                 /// Creates a snapshot from specified tables. Each table's `Value` iterator
// represents a column.                 ///
//                 /// **Ensure the range contains the same number of rows.**
//                 ///
//                 /// * `tx`: Database transaction.
//                 /// * `range`: Data range for columns in tables.
//                 /// * `additional`: Additional columns which can't be straight straightforwardly
// walked on.                 /// * `keys`: Iterator of keys (eg. `TxHash` or `BlockHash`) with
// length equal to `row_count` and ordered by future column insertion from `range`.                 
// /// * `dict_compression_set`: Sets of column data for compression dictionaries. Max size is 2GB.
// Row count is independent.                 /// * `row_count`: Total rows to add to `NippyJar`.
// Must match row count in `range`.                 /// * `nippy_jar`: Snapshot object responsible
// for file generation.                 #[allow(non_snake_case)]
...
//                     additional: Option<Vec<Box<dyn Iterator<Item = Result<Vec<u8>, Box<dyn
// StdError + Send + Sync>>>>>>,                     dict_compression_set: Option<Vec<impl
// Iterator<Item = Vec<u8>>>>,                     keys: Option<impl Iterator<Item =
// ColumnResult<impl PHFKey>>>,                     row_count: usize,
//                     nippy_jar: &mut NippyJar<H>
//                 ) -> ProviderResult<()>
//                     where K: Key + Copy
//                 {
//                     let additional = additional.unwrap_or_default();
//                     debug!(target: "reth::snapshot", ?range, "Creating snapshot {:?} and {} more
// columns.", vec![$($tbl::NAME,)+], additional.len());

//                     let range: RangeInclusive<RawKey<K>> =
// RawKey::new(*range.start())..=RawKey::new(*range.end());

//                     // Create PHF and Filter if required
//                     if let Some(keys) = keys {
//                         debug!(target: "reth::snapshot", "Calculating Filter, PHF and offset
// index list");                         nippy_jar.prepare_index(keys, row_count)?;
//                         debug!(target: "reth::snapshot", "Filter, PHF and offset index list
// calculated.");                     }
...
//                     debug!(target: "reth::snapshot", jar=?nippy_jar, "Generating snapshot
// file.");

//                     nippy_jar.freeze(col_iterators.into_iter().chain(additional).collect(),
// row_count as u64)?;
