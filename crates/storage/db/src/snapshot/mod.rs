...
// /// Given the snapshots directory path, it returns a list over the existing snapshots organized
// by /// [`SnapshotSegment`]. Each segment has a sorted list of block ranges and transaction
// ranges. pub fn iter_snapshots(path: impl AsRef<Path>) -> Result<SortedSnapshots, NippyJarError> {
